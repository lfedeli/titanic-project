...
def drop_na_inputs(*, input_data: pd.DataFrame) -> pd.DataFrame:
    ...
    new_vars_with_na = [
        ...
        if var
        not in config.model_config.categorical_variables
...
class TravelerDataInputSchema(BaseModel):
    pclass: Optional[int]
    sex: Optional[str]
    age: Optional[float]
...
class MultipleTravelerDataInputs(BaseModel):
    inputs: List[TravelerDataInputSchema]
